...
const schema = yup.object().shape({
    ...
    file: yup.mixed()
        ...
        .test("fileType", "Only png, jpg, jpeg files are allowed",
            (value) => {
                if (typeof (value) == 'string') {
                    return value?.length > 0;
                }
    ...
    phone: yup.string().required().matches(PHONE_REGEX, 'Phone number is not valid'),
...
export default function EditProfile() {
    ...
    if (user_data?.account_type == 'user') {
        return (
            <section className="container">
                <Navbar />
                <h1 className="text-center mt-10 font-semibold text-xl ">Update you profile</h1>
                <form onSubmit={handleSubmit(onSubmit)} className="mt-10 flex flex-col gap-5 items-center ">
                    <AvatarUpdateField control={control} name={'file'} errors={errors} watch={watch} />
        ...
        <div className="space-y-2 w-full md:w-auto  " >
            <div className="flex items-center gap-5">
                <Avatar className="w-24 h-24">
                    <AvatarImage src={typeof (avatar) == 'object' ?
                        ACCEPTED_FILE_TYPE.includes(avatar?.type)
                            ? URL.createObjectURL(avatar) : user_data?.avatar
                        : user_data?.avatar
                    } />
